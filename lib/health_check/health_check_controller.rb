    ...
    def index
      ...
      if errors.blank?
        ...
        respond_to do |format|
          format.html { render :plain => HealthCheck.success, :content_type => 'text/plain' }
          format.json { render :json => obj }
          format.xml { render :xml => obj }
          format.any { render :plain => HealthCheck.success, :content_type => 'text/plain' }
        end
      else
        msg = "health_check failed: #{errors}"
        obj = { :healthy => false, :message => msg }
        respond_to do |format|
          format.html { render :plain => msg, :status => HealthCheck.http_status_for_error_text, :content_type => 'text/plain' }
          ...
          format.any { render :plain => msg, :status => HealthCheck.http_status_for_error_text, :content_type => 'text/plain' }
        end
