...
Install health_check as middleware if you want the option to skip processing by later entries in the Rails middleware stack
(eg to ignore DB errors from QueryCache). Include "middleware" in the list of checks to have the checks processed by middleware
health check. The middleware check will fail if you have not installed health_check as middleware.
