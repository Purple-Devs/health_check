...
Health check by default outputs in plain/text format, but you can also request:
* html - with white writting on red or green background (useful for a manual status page with small iframes for each server)
* json - in json format
* xml - in xml format
* txt (default) - in plain text format
...
If max_age > 0 and basic_auth_username is not set then the page is marked as public (otherwise it is marked as private).

Rails also sets 'max-age=0' and 'must-revalidate' for private pages.
