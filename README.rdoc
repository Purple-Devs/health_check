...
specify full or include email in the list of checks or vary the smtp settings
...
health_check provides various monitoring URIs 
    ...
    % curl localhost:3000/health_check/full
    success

    % curl localhost:3000/health_check/site_database_email_migrations
    success
...
Installation as a gem is still under development
...
* migrations - checks that the database migration level matches that in db/migrations
...
Copyright (c) 2010 Ian Heggie, released under the MIT license. See LICENSE for details.

== Similar projects

* fitter_happier plugin by atmos - plugin with similar goals, but not compatable with uptime, and does not check email gateway 
