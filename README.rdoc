...
Health_check will now respond with an encoded hash object if json or xml is requested via the HTTP Accept header or by adding .json or .xml to the url. To force plain text, add .html to the url.
