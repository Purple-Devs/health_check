...
* ~> 3.0 for Rails 5.x and 6.x (etc until there is a proven need to split off another branch),
* ~> 2.0 for Rails 4.x,
* ~> 1.7 for Rails 3.x,
* ~> 1.6.3 for Rails 2.3
...
To change the configuration of health_check, create a file `config/initializers/health_check.rb` and add a configuration block like:
    ...
    HealthCheck.setup do |config|
      ...
      # You should only turn this on where there is no public access to this url as it may leak private information
      mattr_accessor :include_error_in_response_body
      self.include_error_in_response_body = false

      # error message is always logged to rails log
