...
* mongoid - checks Mongoid connectivity to MongoDB
...
To change the configuration of health_check, create a file `config/initializers/health_check.rb` and add a configuration block like:
    ...
    HealthCheck.setup do |config|
      ...
      # aws endpoint to test connectivity - required only if s3 check used
      config.aws_endpoint = 'http://localhost:9000'
      ...
      config.mongoid_server_selection_timeout = 3
    end
